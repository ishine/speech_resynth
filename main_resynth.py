...
from src.flow_matching.preprocess import extract_features, resample, tokenize_dataset
...
class TaskRunner:
    ...
    def tokenize_dataset(self, config: str = "configs/unit2speech/mhubert-expresso-2000.yaml"):
        config = OmegaConf.load(config)
        tokenize_dataset(config)

    def train_bigvgan(self, config: str = "configs/unit2speech/mhubert-expresso-2000.yaml"):
        config = OmegaConf.load(config)
    ...
    def synthesize(self, config: str = "configs/unit2speech/mhubert-expresso-2000.yaml"):
        config = OmegaConf.load(config)
        synthesize(config)
